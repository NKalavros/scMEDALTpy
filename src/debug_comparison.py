...
def main():
    ...
    # Compare binned CNV
    # Find the binned CNV file in Python output
    py_cnv_files = [f for f in os.listdir(py_dir) if f.startswith('2_') and '_bin_' in f]
    if py_cnv_files:
        py_cnv_path = os.path.join(py_dir, py_cnv_files[0])
        # For R, we need to find the corresponding file
        # This might need adjustment based on actual R output structure
        compare_binned_cnv(r_tree_path, py_cnv_path)  # placeholder
