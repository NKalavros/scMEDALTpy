...
depthFunction <- function(cell,cellTree){
  ...
  # Handle case where root is not found or multiple roots
  if (length(root) == 0){
    # If no root found, return 0 for all cells
    return(0)
  }
  if (length(root) > 1){
    # If multiple roots, use the first one
    root = root[1]
  }
  ...
  if (cell == root){
    ...
    while(1){
      ...
      if (length(cell1) == 0){
        # If no parent found, this cell might be disconnected
        return(0)
      }
      if (cell1==root){
        break
