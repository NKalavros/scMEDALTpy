...
def update_dege(rg, root):
    for node in rg:
        if not node == root:
            min_val = float('inf')  # Python 3: avoid shadowing built-in 'min'
...
def compute_rdmst(g, root):
    ...
    if root not in g:
        print("The root node does not exist")  # Python 3: print function
        return

    distances = bfs(g, root)
    for node in g:
        if distances[node] == float('inf'):
            print("The root does not reach every other node in the graph")  # Python 3: print function
            return
